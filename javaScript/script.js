...
clickLi.forEach((li, i) => {
    li.addEventListener("click", () => {
        dropdownLink[i]?.classList.toggle("hidden");
        dropdownLink.forEach((dl, idx) => { if(idx!==i) dl.classList.add("hidden") });
    });
});
...
menu?.addEventListener("click", () => { sideBar.style.transform = "translateX(0)"; disableScroll(); });
closeIcon?.addEventListener("click", () => { sideBar.style.transform = "translateX(800px)"; enableScroll(); });
...
const Nav = document.querySelector('nav');
document.addEventListener("scroll", () => {
    Nav?.classList.toggle("scroll-nav", document.documentElement.scrollTop > 70);
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadPage(pageId) {
        ...
        if(newPage){
            ...
            newPage.classList.add('active');
            currentPage = pageId;
            navbar.style.visibility = pageId === 'home' ? 'visible' : 'hidden';
            navbar.style.position = pageId === 'home' ? 'fixed' : 'absolute';
        }
        footerItems.forEach(item => {
            const isActive = item.dataset.page === pageId;
            item.classList.toggle('active', isActive);
            const indicator = item.querySelector('.indicator');
            if (indicator) indicator.style.width = isActive ? '80%' : '0';
        });
        history.pushState({page: pageId}, '', `#${pageId}`);
    }

    footerItems.forEach(item => item.addEventListener('click', () => loadPage(item.dataset.page)));
    ...
    // Go to buttons
    const buttons = [
        {btn:'#goLibraryBtn', page:'library'},
        {btn:'#goSearch', page:'search'},
        {btn:'#goProfile', page:'profile'},
        {btn:'#goHome', page:'home'},
        {btn:'#goHome2', page:'home'},
        {btn:'#goHome3', page:'home'}
    ];
    buttons.forEach(b => {
        const el = document.querySelector(b.btn);
        if(el) el.addEventListener('click', ()=>loadPage(b.page));
    });
});
...
window.addEventListener("DOMContentLoaded", () => {
    const ua = navigator.userAgent.toLowerCase();
    if (ua.includes("median")) {
        document.querySelector("#download-but")?.remove();
...
const text = document.getElementById("wave-text");
if(text) text.innerHTML = text.textContent.split("").map((c,i)=>`<span style="--i:${i}">${c}</span>`).join("");
...
// initial loader دقیقاً تا بارگذاری کامل صفحه
window.addEventListener("load", hideInitialLoader);

// صفحات داخلی
document.addEventListener("readystatechange", () => {
    pageLoader.style.display = document.readyState === "loading" ? "flex" : "none";
});
window.addEventListener("load", () => pageLoader.style.display = "none");
window.addEventListener("pageshow", () => pageLoader.style.display = "none");
window.addEventListener("pagehide", () => pageLoader.style.display = "flex");
