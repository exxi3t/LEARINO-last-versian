...
// === Like Percentage ===
const likePercentageElement = document.querySelector('#likePercentage span');
let likePercentage = 85; // Replace with server data
likePercentageElement.textContent = likePercentage;
...
function setupAnimatedBtn(btnId, effectClass) {
    const btn = document.getElementById(btnId);
    if (!btn) return;
    const effect = btn.querySelector(`.${effectClass}`);
    ...
    btn.addEventListener('click', () => {
        ...
        icon.style.color = icon.classList.contains('fas') ? '#e50914' : '';
    });
}
...
// Event delegation for like/dislike/reply
commentList.addEventListener('click', (e) => {
    const likeBtn = e.target.closest('.comment-like');
    const dislikeBtn = e.target.closest('.comment-dislike');
    ...
    // Uncomment to enable reply form toggle
    // const replyBtn = e.target.closest('.comment-reply');
    // if (replyBtn) {
    //     const replyForm = replyBtn.closest('.comment-content').querySelector('.reply-form');
    //     replyForm.style.display = replyForm.style.display === 'none' ? 'block' : 'none';
    //     replyForm.querySelector('textarea').focus();
    // }
});
...
commentSubmit.addEventListener('click', () => {
    ...
    // Send to server via AJAX or blade
    console.log('Send comment to server:', input.value);
    input.value = '';
});
